...
const ProjectCard = ({
  ...
  return <Card className="group hover:shadow-glow hover:scale-105 transition-all duration-300 overflow-hidden">
      {/* Project Image/Icon */}
      <div className="relative h-48 bg-gradient-secondary overflow-hidden">
        {image ? <div className="w-full h-full flex items-center justify-center p-4">
            <img src={image} alt={name} className="w-32 h-32 object-contain rounded-full bg-white p-2" />
      ...
      <CardContent>
        ...
        {/* Contract Address */}
        {contractAddress && <div className="mb-4 p-3 bg-muted/50 rounded-lg">
            <div className="flex items-center justify-between">
              <span className="text-xs text-muted-foreground">Contract Address:</span>
              <Button variant="ghost" size="sm" className="h-6 px-2" onClick={() => navigator.clipboard.writeText(contractAddress)}>
        ...
        <div className="flex flex-col space-y-2">
          <Button variant="yellow" size="sm" className="w-full" disabled={status === "coming-soon"} onClick={handleLaunchApp}>
            {status === "coming-soon" ? "Coming Soon" : "Letsbonk.fun"}
          </Button>
          <div className="flex space-x-2">
            {website && <Button variant="outline" size="sm" className="flex-1" onClick={() => window.open(website, '_blank', 'noopener,noreferrer')}>
